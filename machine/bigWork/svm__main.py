...
# svm=SVM()
# support=svm.fit(X_train,Y_train)
...
svm=SVM()
support=svm.fit(x_train_pca,Y_train)
...
for i in range(len(predict)):
    if predict[i] > 0.0:
        support_xlim.append(x_test_pca[i, 0])
        support_ylim.append(x_test_pca[i, 1])

x = np.arange(-5.0, 8.0, 0.1)
y = (svm.b - svm.w[0] * x) / svm.w[1]  # 由w1*x1+w2*x2+b=0得到x2(即y)=(-b-w1x1)/w2
drawScatter(x_train_pca,Y_train,True,support_xlim,support_ylim,x,y)

plt.figure(figsize=(13,8))
plot_decision_region(x_test_pca,predict,svm,resolution=0.1)
